...
```{r Exercise3}
  ...
  mError<-1.96*sqrt(p*(1-p)/nrow(yrbss))
...
5.  Describe the relationship between `p` and `me`. Include the margin of
  ...
  There seems to be a parabolic relationship between these values.  The margin of error reaches its maximum at a population proportion of 0.50.
...
6.  Describe the sampling distribution of sample proportions at $n = 300$ and 
  ...
  The distribution seems mostly normal. The center is at 0.10 and the values range from 0.04 and 0.17 or so.
...
7.  Keep $n$ constant and change $p$. How does the shape, center, and spread 
  ...
  As $p$ changes, the shape does not change, but the center moves to somewhat match the value for p. $\hat{p}$ is a good point estimate of $p$.  The interval length for the range does not seem to change, but moves along with the measure of center, keeping it consistent.
    
8.  Now also change $n$. How does $n$ appear to affect the distribution of $\hat{p}$?

  As $n$ grows, the interval gets smaller, narrowing down the estimate for $p$. 
* * *
...
9.  Is there convincing evidence that those who sleep 10+ hours per day are more
  ...
  Null Hypothesis: There is no significant difference in strength training between those who sleep 10 hours on a school day and those who do not.

  Alternative Hypothesis: Those who sleep over 10 hours a day are more likely to strength train every day of the week.
...
10.  Let's say there has been no difference in likeliness to strength train every
  ...
  There would be a 5% chance of detecting a change. A type one error is rejecting the null when it is actually true.
...
11.  Suppose you're hired by the local government to estimate the proportion of 
  ...
  To find the standard error with a margin of error at 0.1 at 95% confidence, we divide to get 0.0051.  Using the value $p=0.5$ in our formula for standard error, we get $n=9604$.  There must be 9604 people sampled in this study.
